  ...
  if (!existing1) {
    const schedule1 = await db.land_schedule.create({
      data: {
        ...
        adjacent_colliery: 'MCL-TLC-02', total_area_acres: '42.5000',
    ...
    for (const plot of plots.slice(0, 4)) {
      await db.land_schedule_item.create({ data: { id: randomUUID(), schedule_id: schedule1.id, plot_id: plot.id, annexure_tag: plot.plot_number === 'P-103' ? 'B' : plot.plot_number === 'P-104' ? 'C' : 'A', is_active: true, updt_ts: new Date() } })
    }
  }
  ...
  if (!existing2 && plots.length > 4) {
    const schedule2 = await db.land_schedule.create({
      data: {
        ...
        proposed_by: 'Rajesh Kumar', proposed_by_role: 'unit_office', area_office: 'MCL-Angul Area',
        adjacent_colliery: 'MCL-TLC-02', total_area_acres: '27.1000',
    ...
    for (const plot of plots.slice(4)) {
      await db.land_schedule_item.create({ data: { id: randomUUID(), schedule_id: schedule2.id, plot_id: plot.id, annexure_tag: 'A', is_active: true, updt_ts: new Date() } })
    }
  }
}
