...
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ entityType: string, entityId: string, milestoneId: string }> }
) {
  return withRequestContext(request, async () => {
    ...
    const { entityType, entityId, milestoneId } = await params;
    ...
    try {
      ...
      if (!existing || existing.entity_id !== entityId || existing.entity_type !== entityType) {
