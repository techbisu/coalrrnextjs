...
export async function GET(_req: NextRequest) {
  try {
    const [
      ...
      db.mst_project.findMany({ include: { _count: { select: { land_schedule: true, compensation_payroll: true } } } }),
      db.mst_plot.findMany({ include: { mouza: true } }),
      db.form_i_claim.findMany({ include: { mst_plot: true } }),
      db.compensation_payroll.findMany({ include: { compensation_payroll_line: true, mst_project: true } }),
    ...
    return ok({
      ...
      payrolls: payrolls.map((p) => ({
        ...
        lineCount: p.compensation_payroll_line.length,
      ...
      nomineePools: nomineePools.map((np) => ({
        ...
        contributionCount: np.nominee_pool_contribution.length,
      })),
