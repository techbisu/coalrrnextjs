...
import { CheckCircle2, Edit3, Send, Loader2, Lock, Hash } from 'lucide-react'
import { z } from 'zod'
...
export function NumberInputField({
  ...
  const existingValue = typeof submission?.userInput === 'number'
    ? String(submission.userInput)
    : submission?.userInput?.value != null ? String(submission.userInput.value) : '';
  ...
  const handleSave = async () => {
    // Zod Client-Side Numeric Validation
    let schema = z.coerce.number({ message: 'Please enter a valid number' });

    if (inputSchema?.min != null) {
      schema = schema.min(inputSchema.min, `Value must be at least ${inputSchema.min}`);
    }
    if (inputSchema?.max != null) {
      schema = schema.max(inputSchema.max, `Value must be at most ${inputSchema.max}`);
    }

    const numVal = parseFloat(value);
    if (isNaN(numVal) && isMandatory) {
      setErrorMsg('A numeric value is required');
      return;
    }

    const parseRes = schema.safeParse(numVal);
    if (!parseRes.success) {
      setErrorMsg(parseRes.error.issues[0]?.message || 'Invalid number');
      return;
    }
    ...
    try {
      await onSubmit(ruleId, undefined, { value: parseRes.data, unit: inputSchema?.unit });
      setIsEditing(false);
    } finally {
  ...
  return (
    <div className={`p-4 rounded-xl border transition-all duration-200 ${
      isSatisfied && !isEditing
        ...
        : 'bg-card border-border shadow-sm hover:border-blue-200'
    }`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-semibold text-sm text-foreground">{title}</span>
            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950 dark:text-blue-300 dark:border-blue-800 text-[10px] py-0 h-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="space-y-1">
              <p className="text-sm font-semibold font-mono text-foreground">
                {existingValue} {inputSchema?.unit || ''}
              </p>
          ...
          <div className="space-y-2">
            <div className="relative">
              <Input
                ...
                placeholder={inputSchema?.placeholder || "Enter numeric value..."}
                className="text-sm pr-12 font-mono"
              />
              {inputSchema?.unit && (
