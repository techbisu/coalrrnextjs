...
export class MathProvider implements ICaptchaProvider {
  ...
  private generateEasy() {
    ...
    if (op === '+') {
      ...
      return { challenge: `${a} + ${b}`, expected_answer: String(a + b) }
    }
  ...
  private generateDifficult() {
    ...
    if (type === 1) {
      ...
      const answer = a + b - c
      return { challenge: `${a} + ${b} − ${c}`, expected_answer: String(answer) }
    }
    // Division with clean result
    const divisor = this.randomInt(3, 9)
    const quotient = this.randomInt(5, 15)
    const dividend = divisor * quotient
    return { challenge: `${dividend} ÷ ${divisor}`, expected_answer: String(quotient) }
  }
