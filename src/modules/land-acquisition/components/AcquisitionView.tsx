...
export function AcquisitionView() {
  ...
  const { selectedScheduleId, selectSchedule } = useUiState()
  ...
  if (!selectedScheduleId || !schedule) {
    return (
      <ListView
        ...
        onSelect={(id, code) => {
          selectSchedule(id)
          window.history.pushState(null, '', routes.proposal.details(code))
        }}
        onCreated={(id) => {
  ...
  return (
    <DetailView
      ...
      onBack={() => {
        selectSchedule(null)
        window.history.pushState(null, '', routes.proposal.list())
      }}
      onChanged={() => {
