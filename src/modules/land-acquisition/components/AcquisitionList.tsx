...
function AnnexurePill({ tag, count }: { tag: 'A' | 'B' | 'C'; count: number }) {
  ...
  return (
    <div className={`rounded-md border px-2 py-1 ${meta.color}`} title={`Annexure ${meta.label} (${meta.desc})`}>
      <div className="text-[10px] font-semibold uppercase">Anx. {meta.label}</div>
    ...
    <div className="space-y-6">
      ...
      {schedules.length === 0 ? (
        <Alert>
          ...
          <AlertTitle>{t('no_proposals_title', 'No acquisition proposals yet')}</AlertTitle>
          <AlertDescription>
            {t('no_proposals_desc', 'Create your first land acquisition proposal against a locked project baseline to begin the CL-1 checklist + area/HQ vetting workflow.')}
          </AlertDescription>
        </Alert>
      ) : (
