...
export function DocSidebar({ tree, allDocsCount, onSelectDoc }: DocSidebarProps) {
  ...
  return (
    <aside className="w-full flex flex-col h-full bg-slate-50/90 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800/80 text-slate-800 dark:text-slate-200 select-none">
      ...
      <div className="p-3 border-b border-slate-200/80 dark:border-slate-800/80 bg-white/60 dark:bg-slate-900/40 shrink-0">
      ...
      {/* Navigation Tree: General Documentation Shown FIRST */}
      <div className="flex-1 overflow-y-auto p-3 space-y-4 text-xs scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
        {/* 1. General Documentation (Root Files) - FIRST */}
        {rootFiles.length > 0 && (
          <div className="space-y-1">
            <div className="px-1.5 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-emerald-700 dark:text-emerald-400">
                <BookOpen className="w-3.5 h-3.5 text-emerald-500" />
                <span>General Documentation</span>
              </div>
              <Badge variant="outline" className="text-[9px] py-0 px-1 font-mono text-slate-400">
                {rootFiles.length}
              </Badge>
            </div>
        ...
        {/* 2. Subfolders (Architecture, SOP, Agent Rules, etc.) - SECOND */}
        {rootSubfolders.length > 0 && (
          <div className="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800">
            <div className="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center gap-1.5">
              <Layers className="w-3.5 h-3.5 text-sky-500" />
              <span>Doc Categories & Folders</span>
            </div>
      ...
      {/* Sidebar Footer */}
      <div className="p-2.5 border-t border-slate-200 dark:border-slate-800 bg-white/60 dark:bg-slate-900/60 backdrop-blur text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between font-mono shrink-0">
