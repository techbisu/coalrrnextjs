...
import { Mountain, ShieldCheck, Users, Mail, Phone, Lock, Fingerprint, Loader2, CheckCircle2, ArrowRight, Building2 } from 'lucide-react'
import { cn } from '@/lib/utils'
import { Captcha } from '@/modules/captcha/components/Captcha'
...
export function AuthView() {
  ...
  const [authStep, setAuthStep] = React.useState<'credentials' | 'otp' | 'forget-password'>('credentials')
  ...
  const submitForgetPassword = async () => {
    ...
    try {
      const r = await fetch('/api/auth/forget-password', {
        ...
        body: JSON.stringify({ email: eclForm.email })
      })
      const data = await r.json()
      if (!r.ok) throw new Error(data.error)
      toast.success(data.message)
      ...
      toast.error(e instanceof Error ? e.message : 'Failed to request password reset')
    } finally {
      setLoading(false)
    }
  }
              ...
              >
                <div className="mb-5 grid grid-cols-2 gap-2 rounded-lg bg-muted/40 p-1">
                  <button onClick={() => setPortal('ecl')} className={cn('flex items-center justify-center gap-2 rounded-md py-2 text-sm font-medium transition', portal === 'ecl' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
                    <Building2 className="h-4 w-4" /> ECL Portal
                  </button>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => setAuthStep('forget-password')} className="text-[10px] text-amber-700 hover:underline">Forgot password?</button>
                      </div>
                      <div className="relative mt-1">
                    ...
                    <div className="rounded-md bg-muted/40 p-2.5 text-[11px] text-muted-foreground">
                      ...
                      <ul className="space-y-0.5 font-mono">
                        <li>superadmin@coalrr.in — Super Admin</li>
                      </ul>
                    </div>
                  </div>
                )}
            ...
            {authStep === 'forget-password' && (
              ...
              >
                ...
                <div className="space-y-3 mt-4">
                  ...
                  <Captcha purpose="ecl-forget-password" onVerified={() => setEclCaptchaVerified(true)} />
                  <Button onClick={submitForgetPassword} disabled={loading || !eclForm.email || !eclCaptchaVerified} className="w-full bg-amber-600 hover:bg-amber-700">
                    {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Mail className="h-4 w-4" />} Send Reset Link
                  </Button>
                  <Button variant="ghost" onClick={() => setAuthStep('credentials')} className="w-full text-xs">
                    Back to Login
                  </Button>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  <Button onClick={submitOTP} disabled={loading || otpValue.length !== 6 || expiresIn === 0} className="w-full bg-slate-900 hover:bg-slate-800 text-white shadow-sm transition-all">
                    {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />} Verify &amp; Sign In
                  </Button>
                  <Button variant="ghost" onClick={handleResendOTP} disabled={resending || resendCooldown > 0 || expiresIn === 0} className="w-full text-xs text-muted-foreground hover:text-foreground">
                  ...
                  <Button variant="link" onClick={() => { setAuthStep('credentials'); setOtpValue('') }} className="w-full text-xs text-muted-foreground">
                    Back to login
                  </Button>
                </div>
              </motion.div>
