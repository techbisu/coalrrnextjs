...
import { SectionCard, StatTile, DataTable, StateBadge } from '@/shared/components/coalrr'
import type { Column } from '@/shared/components/coalrr'
          ...
          {data && (
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <AlertRow icon={CheckCircle2} color="violet" label="Total pending reviews" value={data.stats.pendingReviewCount} onClick={() => { setView('workflow-inbox'); window.history.pushState(null, '', routes.workflow.list()); }} />
              </div>
...
function MyPendingTasksList() {
  ...
  if (isLoading) return <div className="text-xs text-muted-foreground animate-pulse">Loading tasks...</div>
  if (!tasks || tasks.length === 0) return null

  return (
    <div className="mt-4 pt-4 border-t border-border">
      <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">My Action Center</h4>
      <ul className="space-y-2">
        {tasks.map(task => (
          <li key={task.id}>
            <button
              onClick={() => {
                if (task.reviewable_type === 'document_signature') {
                  // Actually the reviewable_id is the document_instance.id.
                  // We can route to the proposal or document-engine workspace. 
                  // For now, let's just route to proposals list or a specific view.
                  // Let's route to workflow inbox
                  setView('workflow-inbox')
                  window.history.pushState(null, '', routes.workflow.list())
                }
              }}
              className="flex w-full items-center justify-between gap-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-left transition hover:bg-amber-100 dark:bg-amber-950 dark:border-amber-700"
            >
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-medium text-amber-900 dark:text-amber-100 truncate">
                  {task.reviewable_type === 'document_signature' ? 'Signature Required' : 'Action Required'}
                </span>
                <span className="text-xs text-amber-700 dark:text-amber-300 truncate">Role: {task.role}</span>
              </div>
              <span className="text-[10px] whitespace-nowrap bg-amber-200 text-amber-800 px-1.5 py-0.5 rounded-sm dark:bg-amber-800 dark:text-amber-200">
                {timeAgo(task.entry_ts)}
              </span>
            </button>
          </li>
        ))}
